...
export const projects: Project[] = [
  {
    ...
    name: "AudioReuse",
    slug: "audio-reuse",
    positioning: "本地语音资产复用助手",
    stageLabel: "资产复用",
    status: "In Development",
    progress: "2026.05 起 · 检索库已建立 · GUI 迭代中",
    problem: "近 40 万条历史录音散落在多个交付目录，靠记忆和翻文件夹找不全，能复用的内容常被重新录一遍。",
    method: "把音频、配音文本、ASR 转写和声学特征统一入库，用关键词、语义和相似声音三路召回可复用候选。",
    value: "在排录制计划前先判断资产是否已存在，把重复录制、棚期占用和制作成本压下来。",
    startDate: "2026.05",
  ...
  },
  {
    ...
    name: "AudioCheck",
    slug: "audio-check",
    positioning: "录音棚出档回收检查",
    stageLabel: "出档回收",
    status: "Stable",
    progress: "2026.04 起 · 规则收口 · 稳定复用",
    problem: "每月多个录音棚、多批次出档，靠人工核对哪些录了、哪些漏了、归属哪个棚，容易乱也容易漏。",
    method: "把配音文本、排班表和出档目录三方比对，按棚统计完成率并列出缺失行。",
    value: "把回收状态变成一份可复查、可直接发棚追档的清单，减少遗漏和反复确认。",
    startDate: "2026.04",
  ...
  },
  {
    ...
    problem: "棚交付的 wav 入库前，命名、缺失、格式和内容是否与配音文本一致，逐条人耳核对成本高。",
    method: "按 Excel 顺序和命名规则推导目标名并重命名，ASR 转写与原文比对只作内容侧的辅助提示。",
    value: "在入库前把交付风险拦下来，减少进入工程后才发现问题带来的返工。",
    startDate: "2026.05",
  ...
  },
  {
    ...
    name: "ReaperTools",
    slug: "reaper-tools",
    positioning: "Reaper Lua 脚本组",
    stageLabel: "后期处理",
    status: "Internal Utility",
    progress: "2026.06 起 · Lua v0.4 · 部分脚本可通用化",
    problem: "大批量语音进入 Reaper 后，分轨、Region 命名、对白对齐和交付导表都是重复的手工操作。",
    method: "用一组 Lua 脚本在工程内完成分轨归位、Region 命名与 Glue、双轨对齐和响度导表。",
    value: "把后期整理和交付前检查放进 Reaper 工程里做，减少导出之后再回头返查。",
    startDate: "2026.06",
